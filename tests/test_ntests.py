...
def test_all_modules_have_tests():
    ...
    f = filter(lambda s: inspect.ismodule(
        getattr(hep_spt, s)), hep_spt.__all__)
...
def test_all_members_have_tests():
    '''
    Test that all the direct members in "hep_spt" have at least one test. The
    ...
    def fltr(s): return not s.startswith('_') and not inspect.ismodule(
        getattr(hep_spt, s)) and s not in exclude
